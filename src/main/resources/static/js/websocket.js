...
function fetchAll() {
    ...
    $.get(url + "/fetchAllGroups/"+userId, function (response) {
        ...
        for (let i = 0; i < groups.length; i++) {
            ...
            groupsTemplateHTML = groupsTemplateHTML + '<li class="active" id="child_message" onclick="formMessageLauch('+groups[i]['user_id']+',\''+groups[i]['address']+'\',\'group\')" data-groupid="'+groups[i]['user_id']+'" data-type="group">'+
                '<div class="d-flex bd-highlight">'+
                '<div class="img_cont">'+
                '<img src="https://static.turbosquid.com/Preview/001292/481/WV/_D.jpg" class="rounded-circle user_img">'+
                '<span class="online_icon"></span>'+
                ...
                '<p>'+groups[i]['address']+' is active</p>'+
                '</div>'+
                '</div>'+
                '</li>';
        }
